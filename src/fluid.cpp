...
Fluid::Fluid(const FluidConfiguration &config)
    : viscosity(config.viscosity), density(config.density),
      pressureMultiplier(config.pressureMultiplier),
      smoothingRadius(config.smoothingRadius),

      radius(config.radius), friction(config.friction),
      restitution(config.restitution),

      color(config.color),
      circleMesh({0, 0}, config.radius, config.accuracy, config.color),
      vertices(config.dimensions.x * config.dimensions.y *
                   circleMesh.vertices.size(),
               {.position = {0, 0}, .color = config.color}),

      gridDimensions(config.gridSize.x, config.gridSize.y),
      gridMappings(config.gridSize.x,
                   std::vector<std::vector<SDL_Point>>(
                       config.gridSize.y, std::vector<SDL_Point>())),
      grid(config.gridSize.x, std::vector<std::vector<int>>(
                                  config.gridSize.y, std::vector<int>())) {
  ...
  for (int i = 0; i < gridDimensions.x; i++) {
    for (int j = 0; j < gridDimensions.y; j++) {
      SDL_Point groups[] = {{i - 1, j - 1}, {i + 0, j - 1}, {i + 1, j - 1},
                            {i - 1, j + 0}, {i + 0, j + 0}, {i + 1, j + 0},
                            {i - 1, j + 1}, {i + 0, j + 1}, {i + 1, j + 1}};
      for (auto cellId : groups) {
        if (cellId.x >= 0 && cellId.x < gridDimensions.x && cellId.y >= 0 &&
...
void Fluid::draw(SDL_Renderer *renderer) {
  for (size_t i = 0; i < particles.size(); ++i) {
    ...
    for (size_t j = 0; j < circleMesh.vertices.size(); ++j) {
      ...
      vertices[index + j].position.x =
          circleMesh.vertices[j].position.x + position.x;
      vertices[index + j].position.y =
          circleMesh.vertices[j].position.y + position.y;
    }
  }
...
void Fluid::updateRegion() {
  Vector2 top = particles[0].position - radius;
  Vector2 bottom = particles[0].position + radius;

  for (auto &particle : particles) {
    top.x = std::min(top.x, particle.position.x - radius);
    top.y = std::min(top.y, particle.position.y - radius);

    bottom.x = std::max(bottom.x, particle.position.x + radius);
    bottom.y = std::max(bottom.y, particle.position.y + radius);
  }
  Vector2 size = bottom - top;
  region = {top.x, top.y, size.x, size.y};
}
...
void Fluid::updateGrid() {
  ...
  SDL_FPoint cellSize = {WINDOW_INFO.rect.w / gridDimensions.x,
...
float densityToPressure(float Targetdensity, float density,
                        float pressureMultiplier) {
  float difference = density - Targetdensity;
  // Prevent negative pressure so the fluid doesn't pull itself into artificial
  // clumps
...
void Fluid::updatePressure() {
  for (auto &particle : particles) {
    particle.pressure =
        densityToPressure(density, particle.density, pressureMultiplier);
  }
}
...
void Fluid::reset() {
  for (auto &particle : particles) {
    ...
    particle.gradient = {
        0.0f, 0.0f}; // We use this property to accumulate acceleration
  }
}
