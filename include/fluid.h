...
typedef struct ParticleConfiguration {
  float pressure = 0.0, density = 0.0, friction = 0.01f, restitution = 0.99f,
        mass = 1.0f, radius = 3;

  Vector2 position = {0, 0}, velocity = {0, -20}, gradient = {0, 0},
          gravity = {0, 30};
  ;
...
class Particle {
public:
  float pressure, density, friction, restitution, mass, radius;

  Vector2 position, velocity, gradient, gravity;
  ...
  Particle(ParticleConfiguration config)
      : pressure(config.pressure), density(config.density),
        ...
        mass(config.mass), radius(config.radius), position(config.position),
        velocity(config.velocity), gravity(config.gravity),
...
typedef struct FluidConfiguration {
  ...
  int accuracy = 100;
  SDL_FColor color = {207.0f / 255, 51.0f / 255, 176.0f / 255.0f,
                      1}; // rgba(207, 51, 176, 1)
} FluidConfiguration;
...
public:
  ...
  SDL_FColor color;
  float radius, viscosity, density, friction, restitution, pressureMultiplier,
      smoothingRadius;
  ...
  // Holy Dog Water
  template <typename Func> void gridIteration(Func fn) {
